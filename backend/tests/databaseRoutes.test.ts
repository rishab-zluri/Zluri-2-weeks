...
jest.mock('../src/controllers/databaseController', () => ({
  getInstances: jest.fn((req: Request, res: Response) => res.json({ success: true, data: [] })),
  getInstanceById: jest.fn((req: Request, res: Response) => res.json({ success: true, data: {} })),
  getDatabases: jest.fn((req: Request, res: Response) => res.json({ success: true, data: [] })),
  syncInstance: jest.fn((req: Request, res: Response) => res.json({ success: true })),
  getSyncHistory: jest.fn((req: Request, res: Response) => res.json({ success: true, data: [] })),
  syncAll: jest.fn((req: Request, res: Response) => res.json({ success: true })),
  getBlacklist: jest.fn((req: Request, res: Response) => res.json({ success: true, data: [] })),
  addToBlacklist: jest.fn((req: Request, res: Response) => res.json({ success: true })),
  removeFromBlacklist: jest.fn((req: Request, res: Response) => res.json({ success: true })),
}));

// Mock auth middleware to bypass authentication but keep role checks if needed
jest.mock('../src/middleware/auth', () => {
  const actual = jest.requireActual('../src/middleware/auth') as any;
  return {
    ...actual,
...
describe('Database Routes', () => {
  ...
  describe('Sync Routes', () => {
    describe('POST /api/databases/instances/:instanceId/sync', () => {
      ...
      it('should allow admin to sync instance', async () => {
        ...
        adminApp.use((req: Request, res: Response, next: NextFunction) => {
          req.user = { id: '1', role: 'admin', email: 'admin@test.com', managedPods: [] } as any;
          next();
        });
  ...
  describe('requireAdmin Middleware', () => {
    ...
    it('should reject when user role is undefined', async () => {
      ...
      noRoleApp.use((req: Request, res: Response, next: NextFunction) => {
        req.user = { id: '1' } as any; // No role
        next();
      });
